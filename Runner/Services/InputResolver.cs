...
public class InputResolver : IInputResolver
{
    ...
    public async Task<string> LoadInputByUserSelection(SolutionEntry solution, string? selectedInput, string? customInput)
    {
        ...
        if (customInput != null)
            return await LoadInputFromCWD(customInput);
        ...
        if (!registeredInputs.Any())
            throw new InvalidOperationException($"Cannot resolve input for solution [{solution.Day} {solution.Part} {solution.Variant}] because it has no registered services and a custom input was not provided");

        // Selected input takes next priority
        if (selectedInput != null)
            return await LoadSelectedInput(solution, registeredInputs, selectedInput);
    ...
    public async Task<string> LoadInputForSolution(SolutionEntry solution, IInputFile input) => input.Resolution switch
    {
        ...
        InputFileResolution.Named => await LoadInputByName(input.Path),
        var other => throw new InvalidOperationException($"Unsupported InputFileResolution: {other}")
    };

    private async Task<string> LoadInputFromPathRelative(string inputPath)
        => await LoadInputFromCWD(inputPath);
    private async Task<string> LoadInputFromPathAbsolute(string inputPath)
        => await LoadInputFromCWD(inputPath);
    private async Task<string> LoadInputFromCWD(string inputPath)
        => await File.ReadAllTextAsync(inputPath);
    
    private async Task<string> LoadInputPathEmbeddedResourceAbsolute(string inputPath)
        => await LoadEmbeddedResource(inputPath);
    private async Task<string> LoadInputPathEmbeddedResource(SolutionEntry solution, string inputPath)
        => await LoadEmbeddedResource(inputPath, solution.SolutionType);
    private async Task<string> LoadInputByName(string inputPath)
        => await LoadEmbeddedResource(inputPath, typeof(NamedInputs));
